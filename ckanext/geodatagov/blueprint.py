...
def csv(date=None):
    if date:
        entry = model.Session.query(MiscsTopicCSV).filter_by(date=date).first()
    else:
        entry = (
            model.Session.query(MiscsTopicCSV)
...
datapusher.add_url_rule("/usasearch-custom-feed.xml", view_func=feed)
datapusher.add_url_rule("/topics-csv/{date}", view_func=csv)
datapusher.add_url_rule("/topics-csv", view_func=csv)
