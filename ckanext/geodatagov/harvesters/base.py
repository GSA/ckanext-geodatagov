...
from ckanext.geodatagov.validation import (FGDC1998Schema, FGDC1999Schema,
                                           FGDC2001Schema, FGDC2002Schema,
                                           MinimalFGDCValidator)
from ckanext.harvest.model import HarvestObjectExtra as HOExtra
from ckanext.spatial.harvesters import CSWHarvester, DocHarvester, WAFHarvester
...
def validate_profiles(profile):
    ...
    log.debug(
        "Validate {} ({}) in profiles func {}".format(
            profile, type(profile), validation_profile_names
        )
    )

    if profile not in validation_profile_names:
        log.error(
            "Profile {} not found in {}".format(profile, validation_profile_names)
        )
...
class GeoDataGovHarvester(SpatialHarvester):
    ...
    def get_package_dict(self, iso_values, harvest_object):
        ...
        package_dict = super(GeoDataGovHarvester, self).get_package_dict(
    ...
    def fetch_stage(self, harvest_object):
        ...
        try:
            ...
            self._save_object_error(
                "Error getting the record with GUID %s from %s" % (identifier, url),
                harvest_object,
            )
            return False
        ...
        try:
            ...
            if document_format == "iso":
                ...
                extra = HOExtra(
                    object=harvest_object, key="original_document", value=content
                )
                extra.save()

                extra = HOExtra(
                    object=harvest_object, key="original_format", value=document_format
                )
                extra.save()
            else:
