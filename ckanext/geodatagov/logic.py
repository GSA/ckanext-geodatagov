...
def organization_list(context, data_dict):
    ...
    query_results = (
        ...
        .filter_by(key="organization_type")
        .filter(model.GroupExtra.group_id.in_([group["id"] for group in results]))
        .all()
    )
...
ORG_MAPPING = {
    "natiional-park-service": "nps-gov",
...
def doi_create(context, data_dict):
    ...
    if not owner_org:
        print(
            str(
                ...
                + " Fail to import doi id "
                + new_package["id"]
                + ""
                ". Organization "
                + new_package["organization"]["name"]
                + " does not exist."
            )
        )
        return
    ...
    obj = HarvestObject(
        guid=uuid.uuid4().hex, job=context["harvest_job"], content=context["harvestobj"]
    )
    obj.save()
...
def doi_update(context, data_dict):
    ...
    if source_hash == old_source_hash and old_package.get("state") == "active":
        print(
            str(datetime.datetime.now()) + " No change for doi id " + new_package["id"]
        )
        return
...
def rollup_save_action(context, data_dict):
    ...
    if new_extras_rollup:
        new_extras.append(
            {"key": "extras_rollup", "value": json.dumps(new_extras_rollup)}
        )
...
def fix_dataset(data_dict):
    ...
    if extra_tags is not None:
        ...
        extra_tags = [
            dict(name=tag, display_name=tag) for tag in split_tags(extra_tags)
        ]
