 ...
 <xsl:template name="get_text">
    ...
    <em>
     <xsl:choose>
       ...
       <xsl:when test="$el = &quot;otherprj&quot;">Other Projectionâ€™s Definition:&#32;</xsl:when>
...
<!-- filtered out ds: namespace -->
<xsl:template match="/metadata/Esri">
	<xsl:for-each select="child::*">
		<xsl:choose>
			<!-- ds:search tag is the only one that has children  -->
			<xsl:when test="child::* != '' ">
			</xsl:when>
			<xsl:otherwise>
			  <dt><em><xsl:call-template name="EsriTags">
			     <xsl:with-param name="e1"><xsl:value-of select="."/></xsl:with-param>
			   </xsl:call-template></em></dt>				
				<!-- xsl:apply-templates select="."/ -->				
			</xsl:otherwise>
		</xsl:choose>
	</xsl:for-each>
...
<xsl:template name="EsriTags">
         <xsl:variable name="serviceParams" select="/metadata/Esri/ServiceParam"/>
		 <xsl:variable name="serverUrl" select="/metadata/Esri/Server"/>
        <xsl:call-template name="get_text">
          <xsl:with-param name="el">
            <xsl:value-of select="local-name()"/>
          </xsl:with-param>
		...
		<xsl:choose>
          <xsl:when test="local-name()=&apos;Server&apos; and $serverUrl != &apos;&apos; and $serviceParams != &apos;&apos;"> 
		   <xsl:variable name="serverUrl_">	
		   <xsl:choose>
			   <xsl:when test="not(contains(.,'?'))">
			   		<xsl:value-of select="concat(.,'?',$serviceParams)"/>        		
			   </xsl:when>
			   <xsl:otherwise>
			    	<xsl:value-of select="concat(.,$serviceParams)"/>        		
			   </xsl:otherwise>
			   ...
			   <xsl:if test="$serverUrl_ != &apos;&apos; and starts-with($serverUrl_,'http://')">
		       <xsl:element name="a">
          			<xsl:attribute name="href">
            			<xsl:value-of select="$serverUrl_"/>
          			</xsl:attribute>
