...
class TestWafHarvester(object):
    ...
    def run_gather(self, url, source_config):
        ...
        source = WafHarvestSourceObj(
            url=url, owner_org=self.organization["id"], config=source_config, **sc
        )
    ...
    def get_datasets_from_waf_trim_tags(self):
        ...
        url = (
            "http://127.0.0.1:%s/waf-trim-tags/index.html"
    ...
    def test_extras_rollup(self):
        ...
        new_extras = [
            {"key": key, "value": value} for key, value in list(extras.items())
        ]

        get_action("package_update")(
            context, {"id": package.id, "title": "Test change", "extras": new_extras}
        )
