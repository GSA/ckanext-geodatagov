...
class TestLogic(FunctionalTestBase):
    ...
    def test_rollup_save_action(self):
        ...
        new_extras_rollup = json.loads(
            next(item for item in new_extras if item["key"] == "extras_rollup")["value"]
        )
        ...
        assert (
            "Polygon"
            in next(item for item in new_extras if item["key"] == "spatial")["value"]
        )

        # all others should go into new_extras_rollup
        assert (
            json.loads(rollup_extra["value"])["some_extras_rollup"]
