...
class TestExportCSV(FunctionalTestBase):
    ...
    def create_datasets(self):
        ...
        group1_cat = {
            "key": "__category_tag_{}".format(self.group1["id"]),
            "value": '["g1c1"]',
        }
        self.group2 = factories.Group()
        group2_cat = {
            "key": "__category_tag_{}".format(self.group2["id"]),
            "value": '["g2c1","g2c2"]',
        }
        self.group3 = factories.Group()
        group3_cat = {
            "key": "__category_tag_{}".format(self.group3["id"]),
            "value": '["g3c1","g3c2"]',
        }
        self.group4 = factories.Group()
        group4_cat = {
            "key": "__category_tag_{}".format(self.group4["id"]),
            "value": '["g4c1"]',
        }
        ...
        dataset2 = factories.Dataset(  # NOQA
            title="Dataset 2",
            owner_org=organization["id"],
            groups=[{"name": self.group2["name"]}, {"name": self.group3["name"]}],
            extras=[group2_cat],
        )
        ...
        dataset3 = factories.Dataset(  # NOQA
            title="Dataset 3",
            owner_org=organization["id"],
            groups=[{"name": self.group3["name"]}, {"name": self.group4["name"]}],
            extras=[group3_cat],
        )
        ...
        dataset4 = factories.Dataset(  # NOQA
            title="Dataset 4",
            owner_org=organization["id"],
            groups=[{"name": self.group4["name"]}, {"name": self.group1["name"]}],
            extras=[group4_cat],
        )
    ...
    def test_static_export_group_and_tags(self):
        ...
        test_file = pkg_resources.resource_filename(
            __name__, "/data-samples/datasets_category_tags.json"
        )
        ...
        for result in results:
            assert result[
                "organization"
            ], "Department of Housing and Urban Development == Federal Government"
            assert (
                result["organizationUrl"]
                == "https://catalog.data.gov/organization/hud-gov"
            )
            assert (
                result["harvestSourceUrl"]
                == "https://catalog.data.gov/harvest/991bcaf7-498f-4657-bed2-f6594e1bfbe7"
            )
