...
class TestWafCollectionHarvester(object):
    ...
    def run_gather(self, url, source_config):
        ...
        source = WafCollectionHarvestSourceObj(
            url=url,
    ...
    def get_datasets_from_waf_collection1_sample(self):
        ...
        url = (
            "http://127.0.0.1:%s/waf-collection1/index.html"
            % mock_static_file_server.PORT
        )

        collection_metadata = (
            "http://127.0.0.1:%s/waf-collection1/cfg/SeriesCollection_tl_2013_county.shp.iso.xml"
            % mock_static_file_server.PORT
        )
        config = (
            '{"collection_metadata_url": "%s", "validator_profiles": ["iso19139ngdc"], "private_datasets": false}'
            % collection_metadata
        )
    ...
    def test_waf_collection1_parent_exists(self):
        ...
        parent = call_action(
            "package_show",
            context={"user": "dummy"},
            id=extras["collection_package_id"],
        )
    ...
    def test_waf_collection_transformation_failed(self):
        url = (
            "http://127.0.0.1:%s/waf-collection2/index.html"
            % mock_static_file_server.PORT
        )

        collection_metadata = (
            "http://127.0.0.1:%s/waf-collection2/cfg/SeriesCollection_tl_2013_county.shp.iso.xml"
            % mock_static_file_server.PORT
        )
        config = (
            '{"collection_metadata_url": "%s", "validator_profiles": ["iso19139ngdc"], "private_datasets": false}'
