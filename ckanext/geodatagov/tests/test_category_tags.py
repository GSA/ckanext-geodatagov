...
class TestCategoryTags(object):
    ...
    def setup(cls):
        os.system(
            "PGPASSWORD=ckan psql -h db -U ckan -d ckan -c 'drop extension IF EXISTS postgis cascade;'"
        )
        reset_db()
        os.system(
            "PGPASSWORD=ckan psql -h db -U ckan -d ckan -c 'create extension postgis;'"
        )
        # PY2
        if six.PY2:
        ...
        os.system(
            "gunzip -c /tmp/locations.sql.gz | PGPASSWORD=ckan psql -h db -U ckan -d ckan -v ON_ERROR_STOP=1"
        )
        # echo "Cleaning"
    ...
    def create_datasets(self):
        ...
        self.dataset1 = factories.Dataset(
            owner_org=organization["id"], groups=[{"name": self.group1["name"]}]
        )
        self.dataset2 = factories.Dataset(
            owner_org=organization["id"], groups=[{"name": self.group2["name"]}]
        )
