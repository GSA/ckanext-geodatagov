...
def add_status():
    ...
    headers = (
        "count,count_with_date,server,status_code,error,standard,id,unapproved,url"
    )
    ...
    for row in records:
        ...
        try:
            ...
            if server == "Microsoft-IIS/7.5":
                ...
                "apache" in server.lower()
                or "nginx" in server.lower()
            ...
            if content and response.status_code == 200:
                ...
                if extracted_waf:
                    try:
                        content_doc = requests.get(
                            extracted_waf[0][0], timeout=60
                        ).content
...
apache = (
    parse.SkipTo(parse.CaselessLiteral("<a href="), include=True).suppress()
...
other = parse.SkipTo(
    parse.CaselessLiteral("<a href="), include=True
).suppress() + parse.quotedString.setParseAction(parse.removeQuotes).setResultsName(
    "url"
)


scrapers = {
    "apache": parse.OneOrMore(parse.Group(apache)),
