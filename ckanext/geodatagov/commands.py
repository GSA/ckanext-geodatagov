...
from builtins import chr, range, str, zip
...
class GeoGovCommand(inherit):
    ...
    def import_harvest_source(self, sources_location):
        ...
        try:
            ...
            for row in csv_reader:
                ...
                config = {"ORIGINAL_UUID": row["DOCUUID"][1:-1].lower()}
                ...
                try:
                    harvest_source = logic.get_action(
    ...
    def import_organizations(self, location):
        ...
        for num, row in enumerate(all_rows):
            ...
            org = logic.get_action("organization_create")(  # NOQA F841
                {"model": model, "user": user["name"], "session": model.Session},
                {
                    "name": row["name"],
                    "title": row["title"],
                    "extras": [{"key": "organization_type", "value": row["type"]}],
                },
            )
    ...
    def import_dms(self, url):
        ...
        existing_package_ids = set(
            [
                row[0]
                for row in model.Session.query(model.Package.id).from_statement(
                    """select p.id
                                        from package p
    ...
    def import_doi(self):
        ...
        for page in range(0, int(math.ceil(float(total) / pagination)) + 1):
            ...
            url_list_dataset = url_list_dataset.replace(
                '"start": 0', '"start": ' + str(start)
            )
            try:
        ...
        log.info(
            "Datasets found on remote doi server: %s, on local: %s",
            len(collected_ids),
            len(existing_ids),
        )
        ...
        for num, doi_id in enumerate(ids_to_add):
            ...
            try:
                ...
                new_harvestobj = self.get_doi_harvestobj(
                    url_harvestobj + to_import[doi_id]
                )
            except Exception:
                log.exception(
                    "Error when downlaoding doi id=%s and harvest object %s",
                    doi_id,
                    to_import[doi_id],
                )
    ...
    def db_solr_sync(self):
        ...
        url = (
            config.get("solr_url")
            + "/select?q=*%3A*&sort=id+asc&fl=id%2Cmetadata_modified&wt=json&indent=true"
        )
        ...
        if response != "error":
            ...
            print(
                str(datetime.datetime.now()) + " Deleting records from miscs_solr_sync."
            )
            ...
            print(
                ...
                + " Starting insertion of records in miscs_solr_sync ."
            )
            ...
            for x in range(0, int(math.ceil(old_div(rows, chunk_size))) + 1):
                ...
                response = get_response(
                    url + "&rows=" + str(chunk_size) + "&start=" + str(start)
                )
                f = response.read()
                data = json.loads(f)
                ...
                print(
                    ...
                    + str(
                        start
                        + int(data.get("responseHeader").get("params").get("rows"))
                        - 1
                    )
                    + " of "
                    + str(rows)
                ...
                for x in range(0, len(results)):
                    sql = (
                        """select count(id) as count from package where id = :pkg_id;"""
                    )
                    ...
                    for row in q:
                        if row["count"] == 0:
                            ...
                            model.Session.execute(
                                sql, {"pkg_id": results[x]["id"], "action": "notfound"}
                            )
                            model.Session.commit()
                        else:
                            ...
                            if (
                                str(results[x]["metadata_modified"])[:19]
                                != pkg_dict["metadata_modified"][:19]
                            ):
                                print(
                                    str(datetime.datetime.now())
                                    + " Action Type : outsync for Package Id: \
                                    "
                                    + results[x]["id"]
                                )
                                print(
                                    " " * 26
                                    + " Modified Date from Solr: "
                                    + str(results[x]["metadata_modified"])
                                )
                                print(
                                    " " * 26
                                    + " Modified Date from Db: "
                                    + pkg_dict["metadata_modified"]
                                )
                                sql = """delete from miscs_solr_sync where pkg_id = :pkg_id;"""
                                model.Session.execute(sql, {"pkg_id": results[x]["id"]})
                                sql = """insert into miscs_solr_sync (pkg_id, action) values (: pkg_id, : action);"""
                                model.Session.execute(
                                    sql,
                                    {"pkg_id": results[x]["id"], "action": "outsync"},
                                )
                                model.Session.commit()
                            else:
                                sql = """delete from miscs_solr_sync where pkg_id = :pkg_id;"""
                                ...
                                model.Session.execute(
                                    sql,
                                    {"pkg_id": results[x]["id"], "action": "insync"},
                                )
                                model.Session.commit()

                start = (
                    int(data.get("responseHeader").get("params").get("start"))
                    + chunk_size
                )
    ...
    def harvest_job_cleanup(self):
        if p.toolkit.check_ckan_version(min_version="2.8"):
            print(
                "Task removed since new ckanext-harvest include ckan.harvest.timeout to mark as finished stuck jobs"
            )
            return

    def harvest_object_relink(self, harvest_source_id=None):
        print(
            "%s: Fix packages which lost harvest objects for harvest source %s."
            % (
        ...
        if harvest_source_id:
            ...
            results = model.Session.execute(
                sql, {"harvest_source_id": harvest_source_id}
            )
        else:
        ...
        for id in pkgs_problematic:
            ...
            if result:
                print(
                    "%s: %s/%s id %s fixed. Now pushing to solr... "
                ...
                print(
                    "%s: %s/%s id %s has no valid harvest object. Need to inspect mannully. "
                    % (datetime.datetime.now(), count, total, id)
                )

        if not pkgs_problematic:
            print(
                "%s: All harvest objects look good. Nothing to do. "
                % datetime.datetime.now()
            )
    ...
    def export_group_and_tags(packages, domain="https://catalog.data.gov"):
        ...
        for pkg in packages:
            ...
            package["organizationUrl"] = (
                domain + "/organization/" + pkg.get("organization").get("name")
            )
    ...
    def export_csv(self, domain="https://catalog.data.gov"):
        ...
        while True:
            ...
            query = logic.get_action("package_search")(
                {"model": model, "ignore_auth": True}, data_dict
            )
            ...
            result = result + GeoGovCommand.export_group_and_tags(
                packages=packages, domain=domain
            )
        ...
        entry = model.Session.query(MiscsTopicCSV).filter_by(date=date_suffix).first()
        if not entry:
            # create the empty entry for the first time
            entry = MiscsTopicCSV()
    ...
    def jsonl_export(self):
        ...
        message = "{0:.19} jsonl is being generated, {1} pages, total {2} datasets to go.".format(
            str(datetime.datetime.now()), pages, count
        )
        print(message)
        for i in range(pages):
            message = "{0:.19} doing page {1}/{2}...".format(
                str(datetime.datetime.now()), i + 1, pages
            )
            print(message)
        ...
        if bucket_name is not None:
            print("Sending to s3 ...")
            bucket = get_s3_bucket(bucket_name)
            bucket_path = config.get(
                "ckanext.geodatagov.jsonlexport.aws_storage_path", ""
            )
    ...
    def metrics_csv(self):
        ...
        metrics_csv = model.Session.execute(
            sql_METRICS_CSV, {"start_date": start_date, "end_date": end_date}
        )
        ...
        upload_to_key(bucket, path, "%smetrics-%s.csv" % (bucket_path, end_date))
    ...
    def update_dataset_geo_fields(self):
        ...
        if search_backend != "solr":
            raise ValueError(
                "Solr is not your default search backend (ckanext.spatial.search_backend)"
            )
        ...
        for dataset in datasets:
            c += 1
            print(
                "Transforming {}/{}: {}. {} skipped, {} failed, {} transformed".format(
                    c, total, dataset.name, skipped, failed, transformed
                )
            )
            ...
            if old_spatial is None:
                results["datasets"][dataset.id][
                    "skip"
                ] = "No rolled up spatial extra found"
                skipped += 1
                continue
            ...
            for extra in extras:
                if extra["key"] == "spatial":
                    if old_spatial != extra["value"]:
                        ...
                        results["datasets"][dataset.id]["transformation"] = [
                            old_spatial,
                            new_spatial,
                        ]
                    else:
        ...
        print(
            "Final results {} total datasets. {} skipped, {} failed, {} transformed".format(
                total, skipped, failed, transformed
            )
        )
