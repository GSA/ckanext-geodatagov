...
from ckanext.spatial.validation import BaseValidator, FGDCSchema, XsdValidator
...
class MinimalFGDCValidator(BaseValidator):
    ...
    _elements = [
        ...
        (
            "Identification Citation Originator",
        ...
        (
            "Identification Spatial Domain West Bounding Coordinate",
            "/metadata/idinfo/spdom/bounding/westbc",
        ),
        (
            "Identification Spatial Domain East Bounding Coordinate",
            "/metadata/idinfo/spdom/bounding/eastbc",
        ),
        (
            "Identification Spatial Domain North Bounding Coordinate",
            "/metadata/idinfo/spdom/bounding/northbc",
        ),
        (
            "Identification Spatial Domain South Bounding Coordinate",
            "/metadata/idinfo/spdom/bounding/southbc",
        ),
        (
            "Metadata Reference Information Contact Address Type",
            "/metadata/metainfo/metc/cntinfo/cntaddr/addrtype",
        ),
        (
            "Metadata Reference Information Contact Address State",
...
class FGDCValidator(XsdValidator):
    ...
    @classmethod
    def is_valid(cls, xml):
        xsd_filepath = os.path.join(
            os.path.dirname(__file__), cls._xsd_path, cls._xsd_file
        )
        return cls._is_valid(
            xml, xsd_filepath, "FGDC Schema ({0})".format(cls._xsd_file)
        )
